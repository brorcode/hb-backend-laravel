...
class LoanListServiceTest extends TestCase
{
    ...
    #[DataProvider('loanFiltersDataProvider')]
    public function testLoanListServiceHandleFilters(int $count, array $sequence, string $filterKey, array $filters): void
    {
        Loan::factory()
            ->count($count)
            ->sequence(...$sequence)
        ...
        $this->assertCount($count, Loan::all());
        $this->assertCount(1, $data);
        $this->assertEquals($filters[$filterKey]['value'], $data[0][$filterKey]);
    }

    #[DataProvider('loanSortingDataProvider')]
    public function testLoanListServiceHandleSorting(int $count, array $sequence, array $sorting): void
    {
        Loan::factory()
            ->count($count)
            ->sequence(...$sequence)
    ...
    public static function loanFiltersDataProvider(): array
    {
        return [
            'filter_1' => [
                'count' => 2,
    ...
    public static function loanSortingDataProvider(): array
    {
        return [
            'sorting_1' => [
                'count' => 2,
